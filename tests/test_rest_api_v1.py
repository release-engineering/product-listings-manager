...
class TestProductInfo(object):
    ...
    def test_get_product_info(self, client):
        product_version = '7.3'
        p1 = ProductsFactory(label=self.product_label, version=product_version, variant='Client')
        p2 = ProductsFactory(label=self.product_label, version=product_version, variant='Server')
...
class TestProductListings(object):
    ...
    def test_get_product_listings(self, mock_koji_session, mock_get_build, client):
        mock_get_build.return_value = {'id': 1, 'package_name': self.pkg_name, 'version': self.pkg_version, 'release': self.pkg_release}

        # mock result of koji listRPMs() API
        debuginfo_pkg_name = '{}-debuginfo'.format(self.pkg_name)
        debuginfo_nvr = '{}-{}-{}'.format(debuginfo_pkg_name, self.pkg_version, self.pkg_release)
        ...
        t.products.append(p)
        pkg = PackagesFactory(name=self.pkg_name, version=self.pkg_version, arch='x86_64')
        pkg_src = PackagesFactory(name=self.pkg_name, version=self.pkg_version, arch='src')
    ...
    @patch('product_listings_manager.products.get_build')
    @patch('product_listings_manager.products.get_koji_session')
    def test_get_product_listings_src_only(self, mock_koji_session, mock_get_build, client):
        mock_get_build.return_value = {'id': 1, 'package_name': self.pkg_name, 'version': self.pkg_version, 'release': self.pkg_release}
        ...
        # Create products, trees, packages records in db
        variant = 'EXTRAS-6'
        p = ProductsFactory(label=self.product_label, variant=variant, allow_source_only=False)
        t = TreesFactory(arch='x86_64')
        t.products.append(p)
        pkg_src = PackagesFactory(name=self.pkg_name, version=self.pkg_version, arch='src')
        t.packages.append(pkg_src)
...
class TestModuleProductListings(object):
    ...
    def test_get_module_product_listings(self, mock_get_build, client):
        mock_get_build.return_value = {'extra': {'typeinfo': {'module': {'name': self.module_name, 'stream': self.module_stream}}}}
