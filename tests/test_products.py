...
class TestProduct(object):
    ...
    def test_get_product_info_not_found(self, mock_products_model):
        ...
        assert 'Could not find a product with label: {}'.format(label) == str(excinfo.value)
    ...
    def test_get_overrides(self, mock_overrides_model):
        ...
        mock_overrides_model.query.join.return_value.filter.return_value.all.return_value = [
            OverridesModel(name='fake', pkg_arch='src', product_arch='x86_64', include=True),
            OverridesModel(name='fake', pkg_arch='src', product_arch='ppc64', include=True),
            OverridesModel(name='fake', pkg_arch='x86_64', product_arch='x86_64', include=False)
        ]
    ...
    def test_precalc_treelist(self, mock_trees_model):
        ...
        assert sorted(Products.precalc_treelist('fake-product', '7.5', 'Server')) == sorted([1, 3])
    ...
    def test_get_module_overrides(self, mock_moduleoverrides_model):
        ...
        mock_moduleoverrides_model.query.join.return_value.filter.return_value.filter.return_value.all.return_value = [
            ModuleOverridesModel(name=module_name, stream=module_stream, product=1, product_arch='x86_64'),
            ModuleOverridesModel(name=module_name, stream=module_stream, product=1, product_arch='ppc64le'),
            ModuleOverridesModel(name=module_name, stream=module_stream, product=1, product_arch='s390x'),
        ]
        assert Products.get_module_overrides('fake', 'fake', module_name, module_stream, 'fake') == ['x86_64', 'ppc64le', 's390x']
    ...
    def test_get_product_labels(self, mock_products_model):
        ...
        assert Products.get_product_labels() == [{'label': 'label1'}, {'label': 'label2'}]
...
class TestGetProductListings(object):
    ...
    def test_rpms_not_found(self, mock_get_koji_session):
        ...
        assert "Could not find any RPMs for build: {}".format(build) == str(excinfo.value)
...
class TestGetModuleProductListings(object):
    ...
    @patch('product_listings_manager.products.get_build')
    @patch('product_listings_manager.products.models.Trees')
    def test_getModuleProductListings(self, mock_trees_model, mock_get_build, mock_precalc_treelist, mock_get_product_info, mock_get_module_overrides):
        ...
        assert getModuleProductListings('fake-label', nvr) == {'AppStream-8.0': ['x86_64']}

        # test with overrides
        mock_get_module_overrides.return_value = ['ppc64le']
        assert getModuleProductListings('fake-label', nvr) == {'AppStream-8.0': ['ppc64le', 'x86_64']}
